...
function Install-EasyTier {
    ...
    $scriptDir = $PSScriptRoot          # 仅在 .ps1 文件里有效
    $localZipPath = Join-Path $scriptDir "easytier-windows-x86_64-v2.4.5.zip"

    if (-not (Test-Path $localZipPath)) {
        Write-Host "错误: 本地文件未找到: $localZipPath" -ForegroundColor $c_red
        return
    }

    Write-Host "检测到本地安装包: $localZipPath"
    ...
    # 整理子目录结构
    # Write-Host "整理文件结构..." -ForegroundColor $c_yellow
...
# 1. 如果服务正在运行，先停掉，释放 exe/dll 占用
if (Get-Service -Name $ServiceName -ErrorAction SilentlyContinue) {
    Write-Host "  停止服务以释放文件占用..." -ForegroundColor $c_cyan
    Stop-Service -Name $ServiceName -Force -ErrorAction SilentlyContinue
    Start-Sleep -Seconds 2   # 等待进程完全退出
}

# 2. 找到解压出来的子目录
$subDir = Get-ChildItem -Path $InstallDir -Directory |
          Where-Object { $_.Name -like "easytier-windows-x86_64*" } |
          Select-Object -First 1

if ($subDir) {
    Write-Host "  检测到子目录 $($subDir.FullName)，正在覆盖移动..." -ForegroundColor $c_cyan
    # 强制覆盖（此时文件已不被占用）
    Move-Item -Path (Join-Path $subDir.FullName "*") -Destination $InstallDir -Force
    # 删除空目录
    Remove-Item -Path $subDir.FullName -Recurse -Force -ErrorAction SilentlyContinue
...
# =========================================================
    ...
    Write-Host "--- EasyTier 安装/更新成功! ---" -ForegroundColor $c_green
